...
class Mymap(Map):
    ...
    def possible_actions():
        ...
        if not self.is_blocked() and not self.is_dead_end(new_keeper,"left"):
            possible_actions=possible_actions.append(new_keeper,"a")

        # move to right
        new_keeper = (self.keeper[0]+ 1,self.keeper[1])
        if not self.is_blocked() and not self.is_dead_end(new_keeper,"right"):
            possible_actions=possible_actions.append(new_keeper,"d")

        # move down
        new_keeper = (self.keeper[0],self.keeper[1]-1)
        if not self.is_blocked() and not self.is_dead_end(new_keeper,"down"):
            possible_actions=possible_actions.append(new_keeper,"s")

        # move to left
        new_keeper = (self.keeper[0],self.keeper[1]+1)
        if not self.is_blocked() and not self.is_dead_end(new_keeper,"up"):
            possible_actions=possible_actions.append(new_keeper,"w")
    ...
    def number_possibilities(self,new_box):
        ...
        if not (self.get_tile((new_box[0], new_box[1]+1))== Tiles.BOX or self.get_tile((new_box[0], new_box[1]+1))== Tiles.BOX_ON_GOAL):
            cby+=1
        return counter_walls,counter_boxes